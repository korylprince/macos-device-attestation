...
func New(m mdm.MDM, urlPrefix string, fs filestore.FileStore, cert *x509.Certificate, key *rsa.PrivateKey) *Transport {
	return &Transport{mdm: m, prefix: urlPrefix, fs: fs, cert: cert, key: key}
}

// Place places the token at path on the device with serial
func (m *Transport) Place(token, serial, path string) error {
	...
	fsPath, err := m.fs.Put("payload.pkg", signedPkg)
	if err != nil {
	...
	if err = m.mdm.InstallEnterpriseApplication(serial, manifest); err != nil {
